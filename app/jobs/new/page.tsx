        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {form.customerId && hotels.length > 0 && (
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
              <div className="flex items-center gap-1.5 text-amber-800 text-sm font-medium mb-2">
                <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                </svg>
                {hotels.length === 1 ? 'Hotel hinterlegt' : `${hotels.length} Hotels hinterlegt`}
              </div>
              <div className="space-y-2">
                {hotels.map(hotel => (
                  <div key={hotel.id} className="text-sm text-amber-900">
                    <span className="font-medium">{hotel.name}</span>
                    {hotel.address && <span className="text-amber-700"> · {hotel.address}</span>}
                    {hotel.phone && (
                      <a href={`tel:${hotel.phone}`} className="text-amber-700 hover:text-amber-900 ml-1">· {hotel.phone}</a>
                    )}
                    {hotel.note && <div className="text-xs text-amber-600 mt-0.5 italic">{hotel.note}</div>}
                  </div>
                ))}
              </div>
            </div>
