...
function groupBySectionAndPlant(items: ReportData['checklistItems']) {
...
function ChecklistSection({ title, items }: {
  title: string
  items: ReportData['checklistItems']
}) {
...
export function ServiceReportPDF({ data }: { data: ReportData }) {
  const groups = groupBySectionAndPlant(data.checklistItems)
  ...
  return (
    <Document
      title={`Servicebericht ${data.orderNumber}`}
      author="HETA Filterservice"
      subject="Inspektionsbericht"
    >
